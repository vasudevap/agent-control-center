...
| 5 | `WO-056: Google OAuth Production Client and Redirects` | Accepted - Pending Implementation - ADR-006 Acceptance Required | Await ADR-006 or equivalent accepted callback decision; source preflight found no direct browser-facing callback at the earlier placeholder path | Hosted OAuth redirects work with accepted scopes |
