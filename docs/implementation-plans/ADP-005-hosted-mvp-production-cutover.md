...
| 4 | `WO-056A: Grafley Custom Domain Cutover` | In Progress - Frontend DNS and API Certificate Pending | Netlify and Render custom-domain bindings are configured; API DNS is verified but certificate issuance is in error; frontend DNS remains pending | `https://atlas.grafley.com` and `https://api.atlas.grafley.com` are active or a deferment is explicitly recorded |
