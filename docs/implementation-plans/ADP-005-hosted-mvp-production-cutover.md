...
| 4 | `WO-056A: Grafley Custom Domain Cutover` | In Progress - DNS Provisioning Pending | Netlify and Render custom-domain bindings are configured; Repository Maintainer must provision Grafley CNAME records | `https://atlas.grafley.com` and `https://api.atlas.grafley.com` are active or a deferment is explicitly recorded |
