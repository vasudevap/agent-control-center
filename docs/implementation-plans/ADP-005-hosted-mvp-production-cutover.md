...
| 4 | `WO-056A: Grafley Custom Domain Cutover` | In Progress - DNS Propagation and Certificates Pending | Netlify and Render custom-domain bindings are configured; both Grafley CNAMEs are provisioned; API DNS is verified but certificate issuance is in error; frontend public resolver propagation remains pending | `https://atlas.grafley.com` and `https://api.atlas.grafley.com` are active or a deferment is explicitly recorded |
| 5 | `WO-056: Google OAuth Production Client and Redirects` | Accepted - Pending Implementation - Callback Route Decision Required | Await final domain decision from WO-056A; source preflight found no direct browser-facing callback at the earlier placeholder path | Hosted OAuth redirects work with accepted scopes |
