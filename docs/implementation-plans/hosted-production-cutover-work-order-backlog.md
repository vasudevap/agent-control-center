...
| WO-056A | Grafley Custom Domain Cutover | WO-054, WO-055 hosted provider targets | No | In Progress - DNS Provisioning Pending |
...
- Netlify and Render custom-domain bindings exist. Repository Maintainer must
  provision Grafley DNS CNAME records before DNS/TLS verification and runtime
  environment cutover can continue.
