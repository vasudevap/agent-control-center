...
| ---: | --- | --- | --- | --- |
| 1 | `WO-064: Active Navigation and Synthetic Fixture Quarantine` | Completed - Local Validation Passed | Reduce active navigation, remove misleading controls, and quarantine synthetic fixture behavior | Active product path exposes only honest Agent Visibility MVP destinations |
| 2 | `WO-065: Agent Visibility Schema and Migration Foundation` | Accepted - Authorized, blocked on WO-064 merge | Add ES-009 migration, models, constraints, indexes, and historical/synthetic backfill | PostgreSQL migration and model tests pass with legacy evidence preserved |
| 3 | `WO-066: Owner Enrollment and Agent Credentials` | Accepted - Authorized, blocked on WO-065 completion | Implement owner enrollment, one-time credential issuance, verifier storage, and readiness settings | Owner can create a pending agent and receive a non-recoverable one-time credential |
| 4 | `WO-067: Heartbeat and Execution Ingestion` | Accepted - Authorized, blocked on WO-066 completion | Implement authenticated heartbeat and execution ingestion with replay, bounds, redaction, and rate limits | Independent agents can submit accepted telemetry without Atlas runtime control |
| 5 | `WO-068: Health Evaluator and Alert Lifecycle` | Accepted - Authorized, blocked on WO-067 completion | Implement leased evaluator, derived health, alerts, activity, and freshness readiness | Atlas derives observed health and alert state from accepted telemetry |
| 6 | `WO-069: Live Dashboard Integration` | Accepted - Authorized, blocked on WO-068 completion | Replace active UI fixtures with live Overview, Agents, Agent Detail, Executions, Alerts, and Activity | Active dashboard surfaces render live ES-009 data and required states |
| 7 | `WO-070: Disconnect, Reconnect, Archive, and Credential Closeout` | Accepted - Authorized, blocked on WO-069 completion | Complete lifecycle actions, credential rotation/overlap, rejection, retained history, and UI confirmations | Trust lifecycle is owner-controlled without deleting history or stopping external runtimes |
