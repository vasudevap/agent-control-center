...
| 3 | `WO-066: Owner Enrollment and Agent Credentials` | Completed - Local Validation Passed | Implement owner enrollment, one-time credential issuance, verifier storage, and readiness settings | Owner can create a pending agent and receive a non-recoverable one-time credential |
| 4 | `WO-067: Heartbeat and Execution Ingestion` | Accepted - Authorized, blocked on WO-066 merge | Implement authenticated heartbeat and execution ingestion with replay, bounds, redaction, and rate limits | Independent agents can submit accepted telemetry without Atlas runtime control |
