...
| 2 | `WO-065: Agent Visibility Schema and Migration Foundation` | Completed - Local Validation Passed | Add ES-009 migration, models, constraints, indexes, and historical/synthetic backfill | PostgreSQL migration and model tests pass with legacy evidence preserved |
| 3 | `WO-066: Owner Enrollment and Agent Credentials` | Accepted - Authorized, blocked on WO-065 merge | Implement owner enrollment, one-time credential issuance, verifier storage, and readiness settings | Owner can create a pending agent and receive a non-recoverable one-time credential |
