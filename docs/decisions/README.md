...
- [`ADR-006 - Browser-Mediated Google OAuth Callback Surface`](./ADR-006-browser-mediated-google-oauth-callback-surface.md) - Accepted; selects `https://atlas.grafley.com/oauth/google/callback` as the browser-facing Google OAuth redirect surface, with server-side dashboard callback handling and API-owned provider token exchange.
